...
class ScreenTextMatrix:

    # this will be used in the tokenizer; can get rid of common words like "to", "a", "for", etc
    _word_type_exclusion_list = ["IN", "TO", "DT", "CC"]

    _ocr_file_type = ".txt"
    _matrix_file_type = ".txt"

    def __init__(self, title, text_separator, matrix_file_path=None, ocr_file_path=None):
    ...
    def make_matrix(self):
        ...
        with open(self.ocr_file_path + self.title + self._ocr_file_type, "r") as read_file:
            for line in read_file:
                ...
                #checks for the text_separator, to see what iteration it is on
                elif token_string[0] == self.text_separator:
                    current_iteration = int(token_string[1].strip())
                ...
                #looking at the actual string data
                    ...
                    for word_and_type_pair in tagged_string:
                        ...
                        if word_type in self._word_type_exclusion_list:
                            continue
            ...
            matrix_file = open(self.matrix_file_path + self.title + self._matrix_file_type, "w+")
